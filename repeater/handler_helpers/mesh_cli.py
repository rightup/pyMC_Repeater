...
class MeshCLI:
    ...
    def _cmd_help(self) -> str:
        return "Cmds: ver reboot advert clock password clear stats get set setperm neighbors neighbor.remove tempradio log region. Get: battery ip cpu ssid help"
    ...
    def _cmd_get(self, param: str) -> str:
        ...
        if param == "af":
            ...
            return f"> {self._get_battery_info()}"

        elif param == "ip":
            return f"> {self._get_ip_info()}"

        elif param == "cpu":
            return f"> {self._get_cpu_info()}"

        elif param == "ssid":
            return f"> {self._get_ssid_info()}"
    ...
    def _get_ip_info(self) -> str:
        v4 = self._collect_ip_addrs(["ip", "-o", "-4", "addr", "show", "scope", "global"])
        v6 = self._collect_ip_addrs(["ip", "-o", "-6", "addr", "show", "scope", "global"])
    ...
    def _collect_ip_addrs(self, cmd) -> str:
        try:
    ...
    def _read_ups_battery_percent(self) -> Optional[int]:
        try:
    ...
    def _get_battery_info(self) -> str:
        ...
        for candidate in candidates:
            ...
            if type_file.exists():
                try:
                    p_type = type_file.read_text().strip().lower()
                except OSError:
                    p_type = ""
            if p_type == "battery" or candidate.name.lower().startswith("bat"):
                bat_path = candidate
                break
        ...
        if status_file.exists():
            try:
                ...
                status = ""
        volt_file = bat_path / "voltage_now"
        if volt_file.exists():
            try:
                uv = volt_file.read_text().strip()
                if uv.isdigit():
                    voltage = f"{int(uv) / 1_000_000.0:.2f}V"
            except OSError:
                voltage = ""
